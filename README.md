...
CLI-скрипт строит отчёт `median-coffee` по одному или нескольким CSV-файлам и считает медиану `coffee_spent` по каждому студенту с сортировкой по убыванию. Для быстрого ревью в репозитории есть примеры входных данных в `examples/` и отдельный пример реального запуска в `docs/example-run.txt`.
